...
function addExpertModule(options: Schema): Rule {
    return addRootImport(options.project, ({ code, external }) => code`${external('NxExpertModule', '@aposin/ng-soltracker/config')}`);
}
...
function addAposinStyles(options: Schema, path: string): Rule {
    return updateWorkspace(workspace => {
        ...
        const imagePath = `node_modules/@aposin/ng-soltracker/${path}`;
