...
export async function createApp(runner: SchematicTestRunner, tree: Tree, options = {}): Promise<UnitTestTree> {
    ...
    return runner.runExternalSchematic('@schematics/angular', 'application', { name: 'soltracker-testing', standalone: false, ...options }, tree);
}

export async function createTestLibrary(runner: SchematicTestRunner, tree?: Tree, options = {}): Promise<UnitTestTree> {
    return runner.runExternalSchematic('@schematics/angular', 'library', { name: 'soltracker-testing-library' }, tree);
}
...
export async function addLibrary(options: any, tree: Tree): Promise<UnitTestTree> {
    const soltrackerrunner = new SchematicTestRunner('soltracker', require.resolve('../../collection.json'));
    const tempTree = await soltrackerrunner.runSchematic('ng-add', { type: 'b2c', ...options }, tree);
    return soltrackerrunner.runSchematic('ng-add-setup-project', { type: 'b2c', ...options }, tempTree);
}
 ...
 *       import {Foo} from '@aposin/ng-soltracker';
 *
...
export class SchematicTestSetup {
    ...
    init(): void {
        beforeEach(async () => {
            ...
            this.appTree = await createTestApp(this.runner, this.hostTree, { name: 'soltracker-testing' });
            const testAppTsconfigPath = 'projects/soltracker-testing/tsconfig.app.json';
